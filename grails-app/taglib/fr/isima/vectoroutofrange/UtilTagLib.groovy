...
class UtilTagLib {
    ...
    def paginate = { attrs ->
        ...
        if (laststep != 1) {
            writer << '<div class="pagination-centered">'
            writer << "<ul class=\"${cssClasses}\">"
            // display previous link when not on firststep
            if (currentstep > firststep) {
                linkParams.offset = offset - max
                writer << '<li class="arrow">'
                writer << link(linkTagAttrs.clone()) {
                    '&laquo;'
                }
                writer << '</li>'
            } else {
                writer << '<li class="arrow unavailable">'
                writer << '<a>'
                writer << '&laquo;'
                writer << '</a>'
                writer << '</li>'
            ...
            if (steps && laststep > firststep) {
                ...
                if (beginstep > firststep) {
                    ...
                    writer << '</li>'
                    writer << '<li class="unavailable"><span>...</span></li>'
                }

                // display paginate steps
                (beginstep..endstep).each { i ->
                    if (currentstep == i) {
                        writer << "<li class=\"current\">"
                        writer << "<a>${i}</a>"
                        writer << "</li>";
                    } else {
